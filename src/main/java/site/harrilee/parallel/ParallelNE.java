...
//                .config("spark.master", "local[*]")
    ...
    public double[][] getNewTfc(double[][][] tripRtFunc, double[][] odPs, double[][] curTfc, String tripRtFuncType, int firstThruNode) {
        ...
        Broadcast<double[][]> graphDataBroadcast = this.jsc.broadcast(graph.graph);
        Broadcast<Integer> firstThruNodeBroadcast = this.jsc.broadcast(graph.firstThruNode);
        ...
        JavaRDD<Integer> originsRDD = this.jsc.parallelize(origins);
        this.numPartitions = originsRDD.getNumPartitions();
        Tuple2 reducedParents = originsRDD.map(origin -> {
            Graph graph1 = new Graph(graphDataBroadcast.getValue(), firstThruNodeBroadcast.getValue());
            graph1.dijkstra(origin);
            return new Tuple2(origin, graph1.parents);
        }).reduce((tuple1, tuple2) -> {
            int tuple2Idx = (int) tuple2._1();
            int[][] tuple1_ = (int[][]) tuple1._2();
            int[][] tuple2_ = (int[][]) tuple2._2();
            tuple1_[tuple2Idx] = tuple2_[tuple2Idx];
            return new Tuple2(tuple1._1(), tuple1_);
        });

        graph.parents = (int[][]) reducedParents._2();
